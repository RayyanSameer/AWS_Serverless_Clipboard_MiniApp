//This file handles crypto graphy

// we need to geneate the code 
...
function generateCode() {
   ...
   return Array.from(randomBytes).map(b => chars[b % chars.length]).join('')
    ...
    // b here is the byte 
    // here we transform the bytes into charecters and glues it into a string 

    //what this function does : Take input and covert each byte into a char value and glues them as one string 
    // i learned about modulo bias , that's where certain remainders appear more than others making your algorithim more faster to crack.
...
// the function down here turns a normie string into a chad AES 256 encryption key via KeyDeivation

//Take this session code, add some salt, blend it 100,000 times so it's impossible to guess quickly, and give me back a professional-grade AES-256 key that I can use to encrypt and decrypt my files basically.

async function deriveKey(sessionCode) {
    // ecode session code as bytes 
    ...
    return crypto.subtle.deriveKey(
        {
            ...
            hash: 'SHA-256' // essentialyy the blender 
        },
        keyMaterial,
...
export async function encryptText(text, sessionCode) {
    ...
    // Step 3 - convert to base64 so we can put it in JSON

    const ciphertextB64 = btoa(String.fromCharCode(...new Uint8Array(ciphertext)))
    const ivB64 = btoa(String.fromCharCode(...iv))
    
    // Step 4 - return both, recipient needs both
...
export async function decryptText(encryptedB64, ivB64, sessionCode) {
    try {
        ...
        // 2. Convert base64 back to bytes (Reverse of btoa/atob)
